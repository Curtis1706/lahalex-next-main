"use client"
import React, { type ReactNode, useLayoutEffect, useRef, useCallback, useState } from "react"
...
export const ScrollStackItem: React.FC<ScrollStackItemProps> = ({ children, itemClassName = "" }) => (
  <div
    className={`scroll-stack-card relative w-full max-w-[95vw] sm:max-w-[80vw] md:max-w-[700px] lg:max-w-[800px] min-h-[500px] sm:min-h-[450px] md:min-h-[500px] my-6 sm:my-8 p-4 sm:p-6 md:p-8 rounded-[40px] shadow-[0_0_30px_rgba(0,0,0,0.1)] box-border origin-top will-change-transform ${itemClassName}`.trim()}
    style={{
...
const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    cardsRef.current.forEach((card, i) => {
      ...
      if (isPinned) {
        // Card is pinned - calculate its position in the stack
        translateY = scrollTop - cardTop + stackPositionPx + itemStackDistance * i
      } else if (scrollTop > pinEnd) {
        // Card should follow the scroll after pinning ends
        translateY = pinEnd - cardTop + stackPositionPx + itemStackDistance * i
      }
      // If scrollTop < pinStart, translateY stays 0 (natural position)
  ...
  useLayoutEffect(() => {
    if (!isMounted || (!useWindowScroll && !scrollerRef.current)) return
